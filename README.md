...
`getFifoStatus()`でwatermark、FIFO full、lost packet数、format faultを確認できます。FIFO burstの前後で`FIFO_LOST_PKT_CNT`を検査し、epoch開始時の値から変化した場合はtimestamp continuityを保証できないためFIFO faultとしてbatch全体を拒否します。driverは欠落sampleのtimestampを推測しません。raw delta、ODR change bit、validity flagも診断に利用できます。FIFO temperatureは8-bit値を`raw / 2.07 + 25`で変換します。通常register readの16-bit temperatureは従来どおり`raw / 132.48 + 25`であり、両者を混用しません。
...
FIFO有効時はAccel/Gyro ODRを同一かつ25 Hz～2 kHzにしてください。internal clock、`TMST_RES=0`の16-bit deltaでは12.5 Hzの80 ms intervalに必要な75000 ticksを表現できないため、FIFO + 12.5 Hzは`ESP_ERR_INVALID_ARG`で拒否します。4 kHz以上は45 msのgyro startup中に2048-byte FIFO容量を超えるため拒否します。FIFOの複合readと同じinstanceの他操作は呼出し側でserializeしてください。INT GPIO使用時はDATA_RDYをrouteせず、FIFO threshold/fullだけをstatic semaphoreへ通知します。`waitFifo()`はFIFO countを先に確認し、semaphoreはwake-up hintとしてのみ使用します。ISRはSPI、heap、logging、blockingを行いません。Packet 4 high-resolution、FSYNC、external RTC、Accel-only、Gyro-only、異なるAccel/Gyro ODRには対応しません。Accel/Gyroの`-32768`はv1.6のinvalid markerとしてvalidity flagをfalseにし、通信成功とは区別します。FIFO温度についてv1.6は独立したinvalid markerを規定していないため、対応構成では`temperature_valid=true`です。
