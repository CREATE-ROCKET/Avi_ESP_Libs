...
class Log67Serial {
   ...
   public:
    void setup(char setCommandReturn = 'j', char setCommandDelete = 'd',
               uint64_t setSerialFrequency = 500000);
    void sendSerial2();
...
void Log67Serial::setup(char setCommandReturn, char setCommandDelete,
                        uint64_t setSerialFrequency) {
...
void Log67Serial::stopCommand() { sendFlag = false; }

// マルチタスク用
// SPI Flashのデータを削除しているときのみ使用
// 呼び出し方は以下の通り。適宜呼び出し側の変数を変える
// xTaskCreatePinnedToCore(Log67Serial1.sendTask, "sendTask1", 8192, NULL, 2,
// &taskHandle, 0);
