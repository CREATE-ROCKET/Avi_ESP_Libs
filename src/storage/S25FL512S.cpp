...
namespace {
constexpr uint8_t kRead = 0x13, kWriteEnable = 0x06, kErase = 0x60,
                  kProgram = 0x12, kStatus = 0x05;
}
S25FL512S::~S25FL512S() {
  if (device_)
    (void)end();
}
esp_err_t S25FL512S::begin(SPICREATE &s, int cs, uint32_t f) {
  if (device_)
...
esp_err_t S25FL512S::waitReady(uint32_t timeout) {
...
esp_err_t S25FL512S::erase(uint32_t t) {
  if (!spi_)
    return ESP_ERR_INVALID_STATE;
...
esp_err_t S25FL512S::write(uint32_t a, const uint8_t *d, size_t n, uint32_t t) {
  if (!spi_)
    return ESP_ERR_INVALID_STATE;
...
esp_err_t S25FL512S::read(uint32_t a, uint8_t *d, size_t n) {
  if (!spi_)
    return ESP_ERR_INVALID_STATE;
