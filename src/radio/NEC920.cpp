...
esp_err_t NEC920::begin(uart_port_t port,int baud,gpio_num_t rx,gpio_num_t tx,gpio_num_t reset,gpio_num_t wakeup,gpio_num_t mode){
    ...
    if(port<0||port>=UART_NUM_MAX||baud<=0||!GPIO_IS_VALID_GPIO(rx)||!GPIO_IS_VALID_OUTPUT_GPIO(tx))return ESP_ERR_INVALID_ARG;
    uart_config_t config{};config.baud_rate=baud;config.data_bits=UART_DATA_8_BITS;config.parity=UART_PARITY_DISABLE;config.stop_bits=UART_STOP_BITS_1;config.flow_ctrl=UART_HW_FLOWCTRL_DISABLE;config.source_clk=AVI_UART_DEFAULT_CLOCK;
