...
esp_err_t SPICREATE::begin(spi_host_device_t host, int sck, int miso, int mosi,
  ...
  if (!GPIO_IS_VALID_OUTPUT_GPIO(sck) || !GPIO_IS_VALID_OUTPUT_GPIO(mosi) ||
      !GPIO_IS_VALID_GPIO(miso) || max_transfer_size == 0)
...
esp_err_t SPICREATE::addDevice(const spi_device_interface_config_t &config,
                               int chip_select, Device &device) {
  if (!initialized_)
    return ESP_ERR_INVALID_STATE;
  if (!GPIO_IS_VALID_OUTPUT_GPIO(chip_select))
    return ESP_ERR_INVALID_ARG;
