...
void csSet(spi_transaction_t *t) {
    digitalWrite(static_cast<int>(t->user), HIGH);
    return;
}
...
bool SPICreate::begin(uint8_t spi_bus, int8_t sck, int8_t miso, int8_t mosi,
                      uint32_t f) {
    frequency = f;
...
void SPICreate::sendCmd(uint8_t cmd, int deviceHandle) {
    ...
    comm.length = 8;
    comm.tx_data[0] = cmd;
    comm.user = reinterpret_cast<void *>(CSs[deviceHandle]);
